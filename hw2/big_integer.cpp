...
std::string to_string(big_integer const &one) {
	...
	tmp.sign = 0;
	while (tmp > 0) {
		std::vector<uint32_t> t = (tmp % 10).data;
		s += '0' + (t.size() == 0 ? 0 : t[0]);
		tmp /= 10;
	}
	if (s.size() == 0)
...
big_integer::big_integer() : data(std::vector<uint32_t>()), sign(0) {}
big_integer::big_integer(int x) : data(std::vector<uint32_t>()), sign(0) {
	if (x == 0) {
		return;
	}
	if (x == INT32_MIN) {
...
big_integer::big_integer(uint64_t x) : data(std::vector<uint32_t>()), sign(0) {
	data.push_back((uint32_t) (x & (UINT32_MAX)));
	data.push_back((uint32_t) ((x & (UINT64_MAX)) >> 32));
}

big_integer::big_integer(uint32_t x) : data(std::vector<uint32_t>()), sign(0) {
	if (x != 0)
		data.push_back(x);
}

big_integer::big_integer(std::string const &s) : data(std::vector<uint32_t>()), sign(0) {
	uint64_t sum = 0;
	if (s.size() == 0 || s == "0" || s == "-0") {
		return;
	}
...
big_integer::big_integer(big_integer const &one) : data(std::vector<uint32_t>(one.data)), sign(one.sign) {}
...
std::vector<uint32_t> shortdiv(big_integer const &a, uint32_t b) {
...
void invert(std::vector<uint32_t> &one) {
...
std::vector<uint32_t> convert(big_integer const &one) {
	if (!one.sign)
		return one.data;
	std::vector<uint32_t> tmp = one.data;
	invert(tmp);
...
big_integer deconvert(std::vector<uint32_t> &one) {
	big_integer tmp;
	if (!(one.back() & (1 << 31))) {
		tmp.data = one;
		return tmp;
...
big_integer big_integer::operator~() {
	std::vector<uint32_t> tmp = convert(*this);
	invert(tmp);
...
big_integer &big_integer::operator&=(big_integer const &one) {
	big_integer tmp;
	std::vector<uint32_t> a = convert(*this);
	std::vector<uint32_t> b = convert(one), c(std::max(a.size(), b.size()));
